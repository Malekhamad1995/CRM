...
import {
  ActivitiesRelatedToActivitiesTypeEnum
} from '../../../../../../../Enums';
import {
...
export const ActivitiesManagementDialog = ({
  ...
  const getRelatedLead = useCallback(async () => {
    setLoadings({ id: 'relatedLeads', value: true });

    const relatedLeadId = +GetParams('id');
    const res = await leadDetailsGet({id: relatedLeadId});
    
    if (!(res && res.status && res.status !== 200)) {
    setData({ id: 'relatedLeads', value: (res&&[res])||[] });
