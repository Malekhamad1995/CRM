...
import {
  ReassignLeads, GetAdvanceSearchSaleLeads, GetAllSearchableFormFieldsByFormId, OrganizationUserSearch, lookupItemsGetId, CloseListOfLeads, GetAllContactLeadsAdvanceSearch
} from '../../../Services';
...
export const LeadsSalesView = () => {
  ...
  const [referred, setReferred] = useState({
    by: orderFilter && orderFilter.ReferredByLeadTypeSaleFilter && orderFilter.ReferredByLeadTypeSaleFilter.fullName || null,
    to: orderFilter && orderFilter.ReferredToLeadTypeSaleFilter && orderFilter.ReferredToLeadTypeSaleFilter.fullName || null,
    mediaDetails: orderFilter && orderFilter.MediaDetailsLeadTypeSaleFilter && orderFilter.MediaDetailsLeadTypeSaleFilter.lookupItemName || null
  ...
  const getLeadsData = useCallback(async (f) => {
    ...
    // referredto.name: [{searchType: 2, value: "RE"}]
    if (Status) {
    ...
    const res = await GetAdvanceSearchSaleLeads(filter, body);
  ...
  const detailedCardSideActionClicked = useCallback(
    (actionEnum, activeData) => async (event) => {
      ...
      if (actionEnum === ActionsEnum.reportEdit.key) {
        GlobalHistory.push(
          `/home/lead-sales/edit?formType=${activeData.leadTypeId}&id=${activeData.id}`
        );
      } else if (actionEnum === ActionsEnum.folder.key) {
        GlobalHistory.push(
          `/home/lead-sales/lead-profile-edit?formType=${activeData.leadTypeId}&id=${activeData.id}`
        );
  ...
  }, [activeFormType, filter, filterSearchDto, leadType, Status, leadsTableFilter, orderBy, referred, dateFilter]);
                    ...
                    <div className='agentSection'>
                      <div className='agentSection1'>
                        <AutocompleteComponent
                          ...
                          selectedValues={orderFilter.ReferredByLeadTypeSaleFilter}
                          getOptionSelected={(option) => option.id === orderFilter.ReferredByLeadTypeSaleFilter.id || ''}
                      ...
                      <div className='agentSection1'>
                        <AutocompleteComponent
                          ...
                          data={(allReferred && allReferred.result) || []}
                          selectedValues={orderFilter.ReferredToLeadTypeSaleFilter}
                          getOptionSelected={(option) => option.id === orderFilter.ReferredToLeadTypeSaleFilter.id || ''}
                          onInputChange={(e) => {
                            if (e && e.target && e.target.value)
                              getAllReferred(e.target.value || '');
                          }}
                      ...
                      <div className='agentSection1'>
                        <AutocompleteComponent
                          ...
                          selectedValues={orderFilter.MediaDetailsLeadTypeSaleFilter}
                          getOptionSelected={(option) => option.lookupItemId === orderFilter.MediaDetailsLeadTypeSaleFilter.lookupItemId || ''}
                          data={(allMediaName) || []}
                          displayLabel={(option) =>
