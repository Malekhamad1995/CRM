...
export const TemplateDeleteDialog = ({
  ...
  const deleteHandler = async () => {
    setIsLoading(true);
    const res = await DeleteTemplate(activeItem.templateId);
    if (!(res && res.status && res.status !== 200)) {
      reloadData();
      showSuccess(t(`${translationPath}template-deleted-successfully`));
    } else showError(t(`${translationPath}template-delete-failed`));
    setIsLoading(false);
    isOpenChanged();
  };
  return (
    <DialogComponent
      ...
      dialogContent={(
        <div className='d-flex-column-center'>
          ...
          <span>
            {`${t(`${translationPath}template-delete-description`)}  ${
              activeItem && activeItem.templateName
            }`}
          </span>
        </div>
      )}
