...
export const InvoiceEditDialog = ({
  ...
  useEffect(() => {
    ...
    const paymentType = { lookupItemId: activeItem.paymentTypeId, lookupItemName: activeItem.paymentTypeName } || null;
    const paymentMode = { lookupItemId: activeItem.paymentModeId, lookupItemName: activeItem.paymentModeName } || null;
    const contactType = { lookupItemId: activeItem.invoiceContactTypeId, lookupItemName: activeItem.invoiceContactType } || null;
    const bank = { lookupItemId: activeItem.bankId, lookupItemName: activeItem.bank } || null;
      ...
      dialogContent={(
        <div className='invoice-management-dialog view-wrapper'>
          ...
          <div className='d-flex-v-center-h-end'>
            ...
            <span>:</span>
            <span className='px-1'>{effectedByNumber ?  NumberWithCommas(effectedByNumber) : effectedByNumber}</span>
          </div>
          ...
          <div className='form-item form-item-wrapper'>
            ...
            isAttachedInput
              ...
              onKeyUp={(e) => {
                ...
                const amountDue = +(parsedValue + state.amountExTax);
                const amountDuePercentage = +(+(amountDue / effectedByNumber) * 100);
                setState({
                  id: 'edit',
                  value: {
                 ...
                 <div className='input-container'>
                  <Inputs
                    ...
                    onKeyUp={(e) => {
                      ...
                      const effectedValue = +(
                        (+(effectedByNumber) / 100) * parsedValue);
                      const amountDue = +(effectedValue + state.amountExTax);
                      const amountDuePercentage = +(+(amountDue / effectedByNumber) * 100);
                      setState({
                        id: 'edit',
                        value: {
                          ...state,
                          taxPercentage: +parsedValue,
                          tax: +effectedValue,
                          amountDue,
                          amountDuePercentage,
...
InvoiceEditDialog.defaultProps = {
  activeItem: null,
};
