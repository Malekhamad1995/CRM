...
export const AmountReceived = ({
  ...
  const getLeasingAmountReceivedDetailsByUnitTransactionId = useCallback(async () => {
    // const res = await GetLeasingChequeRequestTransactionInfo(unitTransactionId);
  ...
  useEffect(() => {
    if (unitTransactionId) getLeasingAmountReceivedDetailsByUnitTransactionId();
  }, [getLeasingAmountReceivedDetailsByUnitTransactionId, unitTransactionId]);
  return (
    <div className='amount-received-wrapper childs-wrapper'>
      ...
      <div className='form-item'>
        <div className='details-items-wrapper'>
          ...
          <div className='details-item-wrapper'>
            <span className='details-title'>{t(`${translationPath}tenant`)}</span>
            <span className='details-value'>{amountReceivedDetails.buyer || 'N/A'}</span>
          </div>
          <div className='details-item-wrapper'>
            <span className='details-title'>{t(`${translationPath}landlord`)}</span>
      ...
      <div className='form-item'>
        <div className='details-items-wrapper'>
          <div className='details-item-wrapper'>
            <span className='details-title'>{t(`${translationPath}agency-fee-tenant`)}</span>
            <span className='details-value'>{amountReceivedDetails.agencyFeeSeller || 'N/A'}</span>
          </div>
          <div className='details-item-wrapper'>
            <span className='details-title'>{t(`${translationPath}agency-fee-landlord`)}</span>
