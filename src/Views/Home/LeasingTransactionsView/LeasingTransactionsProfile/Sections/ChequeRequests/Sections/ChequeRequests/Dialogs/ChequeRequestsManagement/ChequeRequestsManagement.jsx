...
export const ChequeRequestsManagement = ({
  unitTransactionId,
  activeItem,
  reloadData,
  isOpen,
  ...
  const schema = Joi.object({
    ...
    accountName: Joi.string()
      ...
      .required()
      .messages({
        'string.base': t(`${translationPath}date-is-required`),
        'string.empty': t(`${translationPath}date-is-required`),
      }),
  ...
  const saveHandler = async (event) => {
    ...
    const res =
      (activeItem &&
        (await UpdateUnitTransactionChequeRequest(activeItem.transactionChequeRequestId, state))) ||
      (await CreateUnitTransactionChequeRequest(state));
    setIsLoading(false);
    if (!(res && res.status && res.status !== 200)) {
      if (activeItem) showSuccess(t`${translationPath}cheque-request-updated-successfully`);
      else showSuccess(t`${translationPath}cheque-request-created-successfully`);
      reloadData();
    } else if (activeItem) showError(t(`${translationPath}cheque-request-update-failed`));
...
ChequeRequestsManagement.propTypes = {
...
ChequeRequestsManagement.defaultProps = {
  activeItem: null,
};
