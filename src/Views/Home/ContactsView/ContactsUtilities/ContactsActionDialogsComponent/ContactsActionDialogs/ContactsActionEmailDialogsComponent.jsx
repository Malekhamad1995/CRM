...
import { FormsIdsEnum, UploaderThemesEnum } from '../../../../../../Enums';
...
import {
  AutocompleteComponent, DialogComponent, Inputs, Spinner,
} from '../../../../../../Components';
import { UploaderFilesComponent } from '../../../../../../SharedComponents/UploaderFilesComponent/UploaderFilesComponent';

export const ContactsActionEmailDialogsComponent = ({
  unitTemplateFile,
  isOpenChanged,
  item,
  translationPath,
  ...
  const [stateLocalfiles, setstateLocalfiles] = useState({
    Localfiles: []
  });
      ...
      setSelected([
        {
          contactsId: item.id,
          contact: {
            first_name: (item && item.allDetails && item.allDetails['Main Information'][1].value) || '',
            last_name: (item && item.allDetails && item.allDetails['Main Information'][2].value) || '',
    ...
    <div className='ContactsActionEmailDialogsComponent w-100 px-3'>
      ...
      <div className='Email-from-wraper'>
        ...
        >
          ...
          {showlink && (
            <button
              className='MuiButtonBase-root btns-icon theme-solid bg-green-dark mx-2 mb-1'
              tabIndex='0'
              type='button'
              onClick={() => {
                try {
                  window.open(`${window.location.origin}/share/UnitCard?id=${unitItem.id}`);
                } catch (error) {
          ...
          <div className='image-gallery-management-dialog view-wrapper'>
            ...
            <div className='w-100 mb-3'>
              <UploaderFilesComponent
                initUploadedFiles={
                  (stateLocalfiles.Localfiles &&
                    stateLocalfiles.Localfiles.length > 0 &&
                    stateLocalfiles.Localfiles.map((datalocal) => ({
                      uuid: datalocal.fileId,
                      fileName: datalocal.fileName,
                      size: datalocal.size,
                      type: datalocal.type,
                      date: datalocal.date,
                      ModifiedDate: datalocal.ModifiedDate,
                    }))) ||
                  []
                }
                uploadedChanged={(files) =>
                  setState({
                    ...
                    files: (files &&
                      files.map((itemW) => ({
                        fileId: itemW.uuid,
                        fileName: itemW.fileName,
                      }))) ||
                      [],
                  })}
                uploadedallFiles={(filess) =>
                  setstateLocalfiles({
                    ...stateLocalfiles,
                    Localfiles: (filess &&
                      filess.map((itemW) => ({
                        ...
                        type: itemW.type,
                        date: itemW.date,
                        ModifiedDate: itemW.ModifiedDate,
                      }))) ||
                      []
                  })}
              />
...
ContactsActionEmailDialogsComponent.propTypes = {
...
ContactsActionEmailDialogsComponent.defaultProps = {
