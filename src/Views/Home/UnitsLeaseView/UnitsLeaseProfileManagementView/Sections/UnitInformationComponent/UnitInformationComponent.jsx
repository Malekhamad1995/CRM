...
export const UnitInformationComponent = ({ viewType, parentTranslationPath, translationPath }) => {
  ...
  const [hiddenTabByOperationType, setHiddenTabByOperationType] = useState(null);
  ...
  const [isHaveLeaseLeadOwner, setIsHaveLeaseLeadOwner] = useState(false);
  ...
  const getUnitLeadOwner = useCallback(async (id) => {
    setIsLoading(true);
    const unitLeadOwner = await GetAllUnitLeadOwners(id);
    if (!(unitLeadOwner && unitLeadOwner.status && unitLeadOwner.status !== 200)) {
      setLeaseLeadOwner(unitLeadOwner);
      setIsHaveLeaseLeadOwner(!!(unitLeadOwner && unitLeadOwner.leaseLeadOwner));
    } else {
      setLeaseLeadOwner(null);
      setIsHaveLeaseLeadOwner(false);
    }
    setIsLoading(false);
  }, []);
  ...
  const saveHandler = async () => {
      ...
      if (firstErrorTapIndex !== -1) {
        setActiveTab(
          (hiddenTabByOperationType !== null &&
            firstErrorTapIndex >= hiddenTabByOperationType[0] && firstErrorTapIndex >= hiddenTabByOperationType[1] &&
    ...
    const totalValues = unitDetails && unitDetails.unit && Object.values(unitDetails.unit).filter(
    ...
    bottomBoxComponentUpdate(
      <div className='d-flex-v-center-h-end flex-wrap'>
        ...
        <PermissionsComponent
          ...
          permissionsId={!isPropertyManagementView ? UnitsLeasePermissions.EditUnitDetails.permissionsId : UnitPermissions.EditUnitDetails.permissionsId}
        >
    ...
    <div className='units-information-wrapper childs-wrapper b-0'>
      ...
      <div className='tabs-content-wrapper'>
        {formData && formData &&
          formData[activeTab] &&
          formData[
            (hiddenTabByOperationType && activeTab === 4 && activeTab + 2) || (hiddenTabByOperationType !== null &&
      ...
      {isHaveLeaseLeadOwner && (
        <LandlordInfo
          leaseLeadOwner={leaseLeadOwner && leaseLeadOwner.leaseLeadOwner}
          parentTranslationPath={parentTranslationPath}
          translationPath={translationPath}
        />
      )}
