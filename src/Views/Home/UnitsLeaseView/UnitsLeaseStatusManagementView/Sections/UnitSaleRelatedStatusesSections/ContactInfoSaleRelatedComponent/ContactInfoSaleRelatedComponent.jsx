import React, {
 ...
 export const ContactInfoSaleRelatedComponent = ({
   ...
   const [isOpenCreateLeadDialog, setIsOpenCreateLeadDialog] = useState(false);
   ...
   const getLeads = useCallback(async (contactId) => {
     ...
     const response = await GetAllLeadsByContactId({ pageIndex: 0, pageSize: 2500, referredTo: state.agentId }, contactId);
     if (!(response && response.status && response.status !== 200)) {
       setLeads({
         result: (response && response.result) || [],
         totalCount: (response && response.totalCount) || 0,
       ...
       onStateChanged({
         ...
         value: [
           ...(state.contacts || []),
           {
             contactId: localState.contact.contactsId,
             leadId: localState.lead.leadId,
             ownershipPercentage: persantage,
           },
         ],
       });
     }
   ...
   const getTotal = () =>
     selected.tableContacts.reduce((total, contact) => total + contact.ownershipPercentage, 0);
...
if (AgentInformation !== null || undefined)
   ...
   return (
     <div className='unit-status-contact-info-wapper childs-wrapper'>
       ...
       />
       {
         isOpenCreateLeadDialog && (
           <CreateLeadDialog
             open={isOpenCreateLeadDialog}
             onSave={() => {
               getLeads(localState.contact && localState.contact.contactsId);
           }}
             leadData={getLeadJson()}
             close={() => {
           setIsOpenCreateLeadDialog(false);
         }}
           />
 ...
 ContactInfoSaleRelatedComponent.propTypes = {
 ...
 ContactInfoSaleRelatedComponent.defaultProps = {
