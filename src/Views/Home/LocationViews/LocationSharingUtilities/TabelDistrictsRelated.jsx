...
import { GetDistrictsLookups } from '../../../../Services';


export const TabelDistrictsRelated = ({
  ...
  const tableActionClicked = useCallback((item) => {
    GlobalHistory.push(`/home/District/edit?id=${item.lookupsId}&lookupItemName=${item.lookupItemName}`);
  });
  ...
  const getAllMyReferrals = useCallback(async () => {
    setIsLoading(true);
    const res = await GetDistrictsLookups(filter);
