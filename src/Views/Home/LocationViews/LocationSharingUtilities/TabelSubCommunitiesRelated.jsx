...
import { GetSubCommunitiesLookups } from '../../../../Services';


export const TabelSubCommunitiesRelated = ({
  ...
  const tableActionClicked = useCallback((item) => {
    GlobalHistory.push(`/home/SubCommunitie/edit?id=${item.lookupsId}&lookupItemName=${item.lookupItemName}`);
  });
  ...
  const getAllMyReferrals = useCallback(async () => {
    setIsLoading(true);
    const res = await GetSubCommunitiesLookups(filter);
              ...
              headerData={[
                {
                  id: 1,
                  isSortable: true,
                  label: (`${translationPath}subcommunities`),
