...
import { GetCitiesLookups } from '../../../../Services';


export const TabelCityRelated = ({
  ...
  const tableActionClicked = useCallback((item) => {
    GlobalHistory.push(`/home/city/edit?id=${item.lookupsId}&lookupItemName=${item.lookupItemName}`);
  });
  ...
  const getAllMyReferrals = useCallback(async () => {
    setIsLoading(true);
    const res = await GetCitiesLookups(filter);
