...
export const UnitInformationComponent = ({ viewType, parentTranslationPath, translationPath }) => {
  ...
  const [isHaveLeadOwner, setIsHaveLeadOwner] = useState(false);
  ...
  const getUnitLeadOwner = useCallback(async (id) => {
    ...
    if (!(unitLeadOwner && unitLeadOwner.status && unitLeadOwner.status !== 200)) {
      setLeadOwner(unitLeadOwner);
      setIsHaveLeadOwner(!!(unitLeadOwner && unitLeadOwner.leadOwner));
    } else {
      setLeadOwner(null);
      setIsHaveLeadOwner(false);
    }
    setIsLoading(false);
  }, []);
      ...
      if (firstErrorTapIndex !== -1) {
        setActiveTab(
          (hiddenTabByOperationType !== null &&
            firstErrorTapIndex >= hiddenTabByOperationType[0] && activeTab >= hiddenTabByOperationType[1] &&
  ...
  return (
    <div className='units-information-wrapper childs-wrapper b-0'>
      ...
      {isHaveLeadOwner && (
        <SellerInfo
          leadOwner={leadOwner && leadOwner.leadOwner}
          parentTranslationPath={parentTranslationPath}
          translationPath={translationPath}
        />
      )}
