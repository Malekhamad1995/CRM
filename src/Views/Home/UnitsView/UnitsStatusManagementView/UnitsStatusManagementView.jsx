...
export const UnitsStatusManagementView = () => {
  ...
  const getRentRelatedStatusesInit = () => ({
    agentId: null,
  ...
  const getStateInit = useCallback(() => {
    ...
    return {};
  }, [getIsRentRelated, getIsSaleRelated]);
  const getSaleRelatedSchema = () => ({
    ...
    basePrice: Joi.number()
      ...
      .min(1)
      .messages({
        'number.min': t(`${translationPath}base-price-is-required`),
        'number.required': t(`${translationPath}base-price-is-required`),
  ...
  const cancelHandler = useCallback(() => {
    if (unitParams.id && unitParams.from && +unitParams.from === 2)
      GlobalHistory.push(`/home/units/unit-profile-edit?formType=1&id=${unitParams.id}`);
    ...
    const res =
      (getIsSaleRelated() &&
        (await SetUnitAsReserveOrSale({
          ...
          rowVersion: unitParams.rowVersion,
        }))) ||
      (getIsRentRelated() &&
  ...
  useEffect(() => {
    if (unitParams && unitParams.id && state === {})
      setState({ id: 'edit', value: getStateInit() });
  }, [getStateInit, state, unitParams]);
  useEffect(() => {
    ...
    if (status && id && from) {
      ...
      GlobalHistory.push(`/home/units/unit-profile-edit?formType=1&id=${id}`);
    else GlobalHistory.goBack();
  }, []);
  return (
