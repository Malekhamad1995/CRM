...
import { ReminderTypesEnum, ActivitiesRelatedToEnum, LeadsStatusEnum } from '../../../../../../../../Enums';
...
export const ActivitiesManagementDialog = ({
  ...
  const [loadings, setLoadings] = useReducer(reducer, {
    ...
    relatedMaintenanceContracts: false,
  });
  ...
  const [state, setState] = useReducer(reducer, {
    ...
    relatedMaintenanceContractId: id,
  ...
  const getAllRelatedMaintenanceContracts = useCallback(
    async (value) => {
      setLoadings({ id: 'relatedMaintenanceContracts', value: true });
      const res = await GetAllMaintenanceContract({ ...filter, search: value });
      if (!(res && res.status && res.status !== 200)) {
        setData({
          id: 'relatedMaintenanceContracts',
          value: (res && res.result) || [],
        });
      } else {
        setData({
      ...
      setLoadings({ id: 'relatedMaintenanceContracts', value: false });
    },
    [filter]
  );
  ...
  const getMaintenanceContractById = useCallback(async (maintenanceContractId) => {
    setIsLoading(true);
    const res = await GetMaintenanceContract(maintenanceContractId);
    setIsLoading(false);
        ...
        value: {
          ...
          relatedMaintenanceContract:
            (activeItem.relatedMaintenanceContractId &&
              getMaintenanceContractById(activeItem.relatedMaintenanceContractId)) ||
            null,
          reminderPersons,
        },
      });
    }
  }, [activeItem, getPortfolioById, getWorkOrderById, getMaintenanceContractById]);
  const maintenanceContractInit = useCallback(async () => {
    setSelected({ id: 'activeFormType', value: ActivitiesRelatedToEnum.maintenanceContract.key });
    const relatedMaintenanceContract = await getMaintenanceContractById(id);
    setSelected({
      id: 'relatedMaintenanceContract',
      value: relatedMaintenanceContract || null,
    });
  }, [getMaintenanceContractById, id]);

  useEffect(() => {
    if (id && !activeItem) maintenanceContractInit();
  }, [activeItem, id, maintenanceContractInit]);
  useEffect(() => {
